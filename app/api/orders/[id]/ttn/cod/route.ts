...
// платіж) for the full order amount.
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  /* ════════════════════════════════════════════════════════════════════
     STEP 1 — формування ТТН з накладеним платежем на повну суму
     замовлення (отримувач сплачує при отриманні)
  ══════════════════════════════════════════════════════════════════════ */
  const ttnResult = await createOrderTtn(orderId, { codAmount: orderTotal, forbidPostomat: true });
  if (!ttnResult.ok) {
  ...
  log.push({
    ...
    msg: ttnResult.demo
      ? `[ДЕМО-РЕЖИМ] Згенеровано випадковий ТТН ${ttnResult.ttn} — накладений платіж ${orderTotal.toFixed(2)} грн не надсилався реально`
      : `ТТН ${ttnResult.ttn} створено, накладений платіж ${orderTotal.toFixed(2)} грн`,
    data: { ttn: ttnResult.ttn, demo: ttnResult.demo, codAmount: orderTotal },
  });
