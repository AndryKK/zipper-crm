...
// `export const dynamic` route segment config only has an effect from a
// SERVER component — it used to live right here as "force-dynamic", but
// this whole file is "use client", so that export was silently inert.
// Confirmed live: `npm run build` listed /orders as "○ (Static)" with a
// 2-minute revalidate/1-year expire, not "ƒ (Dynamic)" like every other
// force-dynamic route — meaning Next classified this route's client-side
// Router Cache as "static" (5-minute default staleTime, see Next's
// staleTimes docs) instead of "dynamic" (0s, never cached). A manager who
// edited an order's price and navigated back here within that window saw
// the stale pre-edit totals: React reused the previously-mounted
// OrdersPageClient instance from the client cache instead of remounting
// it, so the `useEffect` below never refired to refetch /api/orders.
//
// Fixed the same way app/(admin)/users/page.tsx already does it: a thin
// SERVER component page.tsx (where "force-dynamic" genuinely works) wraps
// this client component in Suspense — see that file for the actual
// dynamic export and Suspense boundary now. useSearchParams() below still
// needs that Suspense ancestor during static generation; unlike before,
// force-dynamic on the server page now also means Next never attempts to
// prerender this tree in the first place.
...
// Named (not default) export — the default export + Suspense boundary now
// live in page.tsx, the thin server component that makes force-dynamic
// actually take effect (see this file's own top-of-file comment).
